...
def test_triangulation_10_points():    
    total_duration = 0
    for _ in range(1000):
        ps = PointSet([Point(random.uniform(0, 10), random.uniform(0, 10)) for _ in range(10)])
...
def test_triangulation_100_points():    
    total_duration = 0
    for _ in range(100):
        ps = PointSet([Point(random.uniform(0, 10), random.uniform(0, 10)) for _ in range(100)])
        start = time.perf_counter()
        Triangulator.triangulate(ps)
        duration = time.perf_counter() - start
        total_duration += duration
...
def test_triangulation_1000_points():    
    total_duration = 0
    for _ in range(10):
        ps = PointSet([Point(random.uniform(0, 100), random.uniform(0, 100)) for _ in range(1000)])
        start = time.perf_counter()
        Triangulator.triangulate(ps)
        duration = time.perf_counter() - start
        total_duration += duration
...
def test_triangulation_10000_points():
    ps = PointSet([Point(random.uniform(0, 1000), random.uniform(0, 1000)) for _ in range(10000)])
